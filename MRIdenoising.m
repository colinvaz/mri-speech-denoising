...
% Run CMF-WISA on noise estimate to get a noise dictionary and encoding
% matrix
...
[w_noise, h_noise] = cmfwisa(noise_spec, config.num_noise_elems, noise_est_opts);
...
for iter = 1 : config.maxiter
    ...
    for t = 1 : config.context_len
        ...
        norm_scaling = sqrt(sum(w_noisy{2}(:, :, t).^2, 1)) ./ sqrt(sum(w_noise(:, :, t).^2, 1));
    ...
    h_noisy{2} = h_noisy{2} .* ((gradient_neg + config.temporal_regularization * ((1./max(h_noisy{2}, eps)) .* ((1/n) * diag(1 ./ curr_var) * (diag(norm_scaling)*target_mean_pos + curr_mean_neg) * ones(num_freq_regularizations * config.num_noise_elems, n) + (1/(n-1)) * diag((target_var + (curr_mean - diag(norm_scaling)*target_mean).^2) ./ curr_var.^2) * (logH_pos + curr_mean_neg * ones(num_freq_regularizations * config.num_noise_elems, n)) + (1/(n-1)) * diag(1 ./ curr_var) * (logH_neg + curr_mean_pos * ones(num_freq_regularizations * config.num_noise_elems, n))))) ./ ...
                             max(gradient_pos + config.temporal_regularization * ((1./max(h_noisy{2}, eps)) .* ((1/n) * diag(1 ./ curr_var) * (diag(norm_scaling)*target_mean_neg + curr_mean_pos) * ones(num_freq_regularizations * config.num_noise_elems, n) + (1/(n-1)) * diag((target_var + (curr_mean - diag(norm_scaling)*target_mean).^2) ./ curr_var.^2) * (logH_neg + curr_mean_pos * ones(num_freq_regularizations * config.num_noise_elems, n)) + (1/(n-1)) * diag(1 ./ curr_var) * (logH_pos + curr_mean_neg * ones(num_freq_regularizations * config.num_noise_elems, n)))), eps));
...
speech_hat_spec = ReconstructFromDecomposition(w_noisy{1}, h_noisy{1}) .* p_noisy{1};
noise_hat_spec = ReconstructFromDecomposition(w_noisy{2}, h_noisy{2}) .* p_noisy{2};

error_spec = noisy_spec - (speech_hat_spec + noise_hat_spec);

error_opts = struct;
error_opts.W_init = w_noisy{2};
error_opts.W_fixed = true;
[~, h_error, ~, cost_error] = cmfwisa(error_spec, num_freq_regularizations * config.num_noise_elems, error_opts);
noise_hat_spec2 = noise_hat_spec + ReconstructFromDecomposition(w_noisy{2}, h_error) .* exp(1j * angle(error_spec));

noise_hat = ReconstructSignal(abs(noise_hat_spec2), angle(noise_hat_spec2), config.nfft, config.win_len, config.win_shift, fs, length(noisy_sig));
% sig_len = length(noise_sig);
% noise_hat = noise_hat * (noisy_sig(1 : sig_len)' * noise_hat(1 : sig_len)) / (noise_hat(1 : sig_len)' * noise_hat(1 : sig_len));
speech_hat = noisy_sig - noise_hat;
% speech_hat = speech_hat / max(abs(speech_hat));
% speech_hat2 = ReconstructSignal(abs(speech_hat_spec), angle(speech_hat_spec), config.nfft, config.win_len, config.win_shift, fs, length(noisy_sig));
